...
module "adverse-incidents-provider-task-def" {
  ...
  service_name          = "adverse-incidents-provider"
  container_image       = "887664210442.dkr.ecr.us-east-1.amazonaws.com/adverse-incidents-provider:latest"
  container_port        = 8000
  cpu                   = 256
  memory                = 512
...
module "adverse-incidents-provider-service" {
  ...
  service_name        = "adverse-incidents-provider-service"
  desired_count       = 1
  container_port      = 8000
