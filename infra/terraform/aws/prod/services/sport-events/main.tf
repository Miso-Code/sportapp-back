...
module "sport-events-tg" {
  ...
  target_group_name              = "sport-events-tg"
...
module "sport-events-task-def" {
  ...
  service_name          = "sport-events"
  container_image       = "887664210442.dkr.ecr.us-east-1.amazonaws.com/sport-events:latest"
  container_port        = 8000
  cpu                   = 256
  memory                = 512
...
module "sport-events-service" {
  ...
  service_name        = "sport-events-service"
  desired_count       = 1
  container_port      = 8000
