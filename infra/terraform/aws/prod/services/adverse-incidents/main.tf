...
module "adverse-incidents-tg" {
  ...
  target_group_name              = "adverse-incidents-tg"
...
module "adverse-incidents-task-def" {
  ...
  service_name          = "adverse-incidents"
  container_image       = "887664210442.dkr.ecr.us-east-1.amazonaws.com/adverse-incidents:latest"
  container_port        = 8000
  cpu                   = 256
  memory                = 512
...
module "adverse-incidents-service" {
  ...
  service_name        = "adverse-incidents-service"
  desired_count       = 1
  container_port      = 8000
