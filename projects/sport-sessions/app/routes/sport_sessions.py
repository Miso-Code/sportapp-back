...
from ..models.schemas.schema import SportSessionFinish, SportSessionStart, SportSessionLocationCreate
from ..services.sport_sessions import SportSessionService
...
async def start_sport_session(sport_session: SportSessionStart, user_id: Annotated[UUID4 | None, Header()] = None, db: Session = Depends(get_db)):
    if user_id:
        if str(user_id) != str(sport_session.user_id):
            return JSONResponse(content={"error": "User is not the owner of the sport session"}, status_code=403)
