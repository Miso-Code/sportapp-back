...
1. To build the application, run `sam build`.
2. To start the local API Gateway, run `sam local start-api`.
...
If the change doesn't modify `template.yaml`, you can only re run the `sam build` step.
...
This project is intended to be deployed using Terraform, not directly through AWS SAM. However, the `template.yaml` file is a valid CloudFormation template and can be used for reference or for local testing with the AWS SAM CLI.
...
An example function that can be used as a target for the API Gateway is provided in `example.py`. This function simply returns the event and context it receives.
It's being used by `sam local start-api` to test the authorizer function.