...
def object_2_dict(context):
    ...
    return {attr: set_val(getattr(context, attr))
            for attr in dir(context) if
            not attr.startswith('__') and not callable(getattr(context, attr))}
